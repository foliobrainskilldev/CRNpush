...
window.loadComponents = async function() {
    try {
        ...
        if (!document.getElementById('globalExportCsvModal')) {
            ...
            <div class="modal-overlay" id="globalExportCsvModal" role="dialog" aria-modal="true" aria-labelledby="globalExportModalTitle">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3 class="modal-title" id="globalExportModalTitle"><span class="iconify" data-icon="ph:file-csv-fill"></span> Export CSV Data</h3>
                        <button class="btn-close" id="closeGlobalExportModal" aria-label="Close modal"><span class="iconify" data-icon="ph:x-bold"></span></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label class="form-label" for="globalCsvFileNameInput">File Name</label>
...
window.initCustomDropdowns = function() {
    ...
    dropdowns.forEach(dropdown => {
        ...
        newHeader.setAttribute('aria-haspopup', 'listbox');
        ...
        newHeader.onclick = toggleDropdown;
...
window.closeAllDropdowns = function() {
    document.querySelectorAll('.dropdown-list').forEach(l => {
        l.classList.remove('show');
        if (l.previousElementSibling) l.previousElementSibling.setAttribute('aria-expanded', 'false');
    });
};
...
window.refreshNotificationsList = async function() {
    ...
    if (activities.length === 0) {
        list.innerHTML = `<li style="text-align:center; color:var(--text-muted);" tabindex="0">No notifications</li>`;
        return;
    }
    ...
    recent.forEach(act => {
        ...
        list.innerHTML += `<li tabindex="0"><strong style="color:var(--primary)">Update:</strong> ${text} <span>${act.date}</span></li>`;
    });
};
...
window.bindNotificationsPanel = async function() {
    ...
    if (btnNotif && panelNotif) {
        ...
        document.addEventListener('click', (e) => {
            if (!panelNotif.contains(e.target) && e.target !== btnNotif) panelNotif.classList.remove('active');
        });
    }
...
window.bindGlobalSearch = function() {
    ...
    input.addEventListener('input', async (e) => {
        ...
        if (fClients.length) html += `<div class="search-category">Clients</div>` + fClients.slice(0, 3).map(c => `<div class="search-item" tabindex="0" onclick="window.location.href='client-detail.html?id=${c.id}'" onkeydown="if(event.key==='Enter')window.location.href='client-detail.html?id=${c.id}'"><div class="search-icon-wrapper search-icon-client"><span class="iconify" data-icon="ph:buildings-fill"></span></div><div class="search-item-info"><div class="search-item-title">${c.name}</div><div class="search-item-meta">${c.contact} • ${c.status}</div></div></div>`).join('');
        if (fLeads.length) html += `<div class="search-category">Leads</div>` + fLeads.slice(0, 3).map(l => `<div class="search-item" tabindex="0" onclick="window.location.href='leads.html?search=${encodeURIComponent(l.name)}'" onkeydown="if(event.key==='Enter')window.location.href='leads.html?search=${encodeURIComponent(l.name)}'"><div class="search-icon-wrapper search-icon-lead"><span class="iconify" data-icon="ph:magnet-fill"></span></div><div class="search-item-info"><div class="search-item-title">${l.name}</div><div class="search-item-meta">${l.company} • ${l.status}</div></div></div>`).join('');
        if (fDeals.length) html += `<div class="search-category">Deals</div>` + fDeals.slice(0, 3).map(d => `<div class="search-item" tabindex="0" onclick="window.location.href='deals.html?search=${encodeURIComponent(d.title)}'" onkeydown="if(event.key==='Enter')window.location.href='deals.html?search=${encodeURIComponent(d.title)}'"><div class="search-icon-wrapper search-icon-deal"><span class="iconify" data-icon="ph:handshake-fill"></span></div><div class="search-item-info"><div class="search-item-title">${d.title}</div><div class="search-item-meta">Value: $${d.value} • ${d.stage}</div></div></div>`).join('');
        if (fTasks.length) html += `<div class="search-category">Tasks</div>` + fTasks.slice(0, 3).map(t => `<div class="search-item" tabindex="0" onclick="window.location.href='tasks.html?search=${encodeURIComponent(t.title)}'" onkeydown="if(event.key==='Enter')window.location.href='tasks.html?search=${encodeURIComponent(t.title)}'"><div class="search-icon-wrapper search-icon-task"><span class="iconify" data-icon="ph:check-square-fill"></span></div><div class="search-item-info"><div class="search-item-title">${t.title}</div><div class="search-item-meta">Due: ${t.dueDate} • ${t.priority}</div></div></div>`).join('');
...
window.app = {
    ...
    bindGlobalKeyboardNavigation: function() {
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                document.querySelectorAll('.modal-overlay.active').forEach(m => m.classList.remove('active'));
                document.querySelectorAll('.dropdown-list.show').forEach(d => {
    ...
    getUnsplashAvatar: function(seedText, type = 'person') {
        ...
        let charCodeSum = 0;
        if (seedText) {
            for (let i = 0; i < seedText.length; i++) charCodeSum += seedText.charCodeAt(i);
        } else {
            charCodeSum = Math.floor(Math.random() * 100);
        }
    ...
    setManagerAvatar: function() {
        document.querySelectorAll('.avatar').forEach(av => {
            ...
            av.setAttribute('aria-label', 'User Profile');
    ...
    initTheme: function() {
        ...
        if (savedTheme) {
            ...
            this.currentTheme = systemPrefersDark.matches ? 'dark' : 'light';
        }
        this.applyTheme();
    ...
    initLanguage: function() {
        ...
        if (savedLang) {
            ...
            this.currentLang = (navigator.language || navigator.userLanguage).toLowerCase().startsWith('pt') ? 'pt' : 'en';
        }
    ...
    applyTranslations: function() {
        ...
        if (flagIcon) {
            flagIcon.setAttribute('data-icon', this.currentLang === 'pt' ? 'flag:br-4x3' : 'flag:us-4x3');
        }
