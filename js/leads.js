(() => {
    ...
    async function initLeads() {
        if (!document.getElementById('leadsTableBody')) return;
    ...
    function bindExportCSVModal() {
        ...
        btnOpenExport.addEventListener('click', () => {
            ...
            const defaultName = `NexusCRM_Leads_${dateStr}`;

            window.openExportModal(
                defaultName,
                "Name your file to save your leads data offline:",
                "Defina o nome do seu arquivo para baixar os seus prospectos:",
                async (fileName) => {
                    const headers =["ID", "Name", "Company", "Source", "Interest", "Status", "Estimated Value", "Email", "Phone"];
                    const rowsData = allLeads.map(lead =>[
                        lead.id || '',
                        lead.name || '',
                        lead.company || '',
                        lead.source || '',
    ...
    function renderLeads(leadsToRender) {
        ...
        leadsToRender.forEach(lead => {
            ...
            tr.setAttribute('aria-label', `Interact with lead ${lead.name}`);
            ...
            tr.innerHTML = `
                ...
                <td class="actions-cell">
                    <button class="btn-action" aria-label="Edit Lead" title="Edit Lead" onclick="window.editLead(${lead.id})"><span class="iconify" data-icon="ph:pencil-simple-fill"></span></button>
                    <button class="btn-action email" aria-label="Send Email" title="Send Email" onclick="window.sendEmail('${lead.email || ''}')"><span class="iconify" data-icon="ph:envelope-simple-fill"></span></button>
                    <button class="btn-action whatsapp" aria-label="Send WhatsApp" title="WhatsApp Message" onclick="window.sendWhatsApp('${lead.phone || ''}')"><span class="iconify" data-icon="ph:whatsapp-logo-fill"></span></button>
                    <button class="btn-action delete" aria-label="Delete Lead" title="Delete Lead" onclick="window.deleteLead(${lead.id})"><span class="iconify" data-icon="ph:trash-fill"></span></button>
                </td>
            `;
