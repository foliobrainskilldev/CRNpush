(() => {
    ...
    function bindExportCSVModal() {
        ...
        btnOpenExport.addEventListener('click', () => {
            ...
            const defaultName = `NexusCRM_Products_${dateStr}`;

            window.openExportModal(
                defaultName,
                "Name your file to save your products data offline:",
                "Defina o nome do seu arquivo para baixar os seus produtos:",
                async (fileName) => {
                    const headers =["ID", "Name", "SKU", "Category", "Price", "Status"];
                    const rowsData = allProducts.map(prod =>[
                        prod.id || '',
                        prod.name || '',
                        prod.sku || '',
                        prod.category || '',
    ...
    function renderProducts(productsToRender) {
        ...
        productsToRender.forEach(prod => {
            ...
            tr.setAttribute('aria-label', `Product ${prod.name}`);
            ...
            tr.innerHTML = `
                ...
                <td class="actions-cell">
                    <button class="btn-action" aria-label="Edit Product" title="Edit Product" onclick="window.editProduct(event, '${prod.id}')"><span class="iconify" data-icon="ph:pencil-simple-fill"></span></button>
                    <button class="btn-action delete" aria-label="Delete Product" title="Delete Product" onclick="window.deleteProduct(event, '${prod.id}')"><span class="iconify" data-icon="ph:trash-fill"></span></button>
                </td>
            `;
