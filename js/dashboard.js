document.addEventListener('DOMContentLoaded', () => {
    // Aguarda que o HTML partilhado (Sidebar/Topbar) seja carregado pelo app.js
...
function bindExportCSVModal(deals) {
    ...
    btnOpen.onclick = () => {
        ...
        document.getElementById('csvFileNameInput').value = `NexusCRM_Report_${dateStr}`;
        modal.classList.add('active');
    };

    const fechar = () => modal.classList.remove('active');
    document.getElementById('closeExportModal').onclick = fechar;
    document.getElementById('cancelExportBtn').onclick = fechar;
    ...
    document.getElementById('confirmExportBtn').onclick = async () => {
        ...
        deals.forEach(deal => {
            const values = headers.map(h => `"${(deal[h] || '').toString().replace(/"/g, '""')}"`);
        ...
        fechar();
        window.hideLoader();
        setTimeout(() => window.showToast("Export completed successfully!", "success"), 200);
    };
}
...
function bindTopControls(lineData, barData, doughnutData) {
    ...
    // Reinicializar dropdowns customizados usando a global app.js
