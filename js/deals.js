(() => {
    ...
    async function initDeals() {
        if (!document.getElementById('colProspecting')) return;
    ...
    function renderDeals(dealsToRender) {
        ...
        stages.forEach(stage => {
            ...
            stageDeals.forEach(deal => {
                ...
                card.setAttribute('aria-label', `Deal: ${deal.title}, Value: ${fmt.format(deal.value)}`);
                
                card.innerHTML = `
                    <div class="card-actions">
                        <button class="card-action-btn" aria-label="Edit Deal" onclick="window.editDeal(event, '${deal.id}')" title="Edit"><span class="iconify" data-icon="ph:pencil-simple-fill"></span></button>
                        <button class="card-action-btn del" aria-label="Delete Deal" onclick="window.deleteDeal(event, '${deal.id}')" title="Delete"><span class="iconify" data-icon="ph:trash-fill"></span></button>
                    </div>
    ...
    function bindExportCSVModal() {
        ...
        btnOpen.onclick = () => {
            ...
            const defaultName = `NexusCRM_Deals_${dateStr}`;

            window.openExportModal(
                defaultName,
                "Name your file to save your deals data offline.",
                "Defina o nome do seu arquivo para baixar os seus negócios:",
                async (fileName) => {
                    const headers =["ID", "Deal Title", "Client ID", "Value", "Stage", "Probability", "Close Date"];
                    const rowsData = allDeals.map(deal =>[
                        deal.id || '',
                        deal.title || '',
                        deal.clientId || '',
                        deal.value || 0,
