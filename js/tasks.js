(() => {
    ...
    function renderTasks(tasksToRender) {
        ...
        tasksToRender.forEach(task => {
            ...
            tr.setAttribute('aria-label', `Task: ${task.title}`);
            ...
            let actionBtnHTML = '';
            if (isDone) {
                actionBtnHTML = `<button class="btn-action btn-undo" aria-label="Undo Task" title="Undo" onclick="window.toggleTaskStatus('${task.id}', 'Pending')"><span class="iconify" data-icon="ph:arrow-u-up-left-fill"></span> ${btnUndoTxt}</button>
                                 <button class="btn-action btn-delete" aria-label="Delete Task" title="Delete Task" onclick="window.deleteTask('${task.id}')"><span class="iconify" data-icon="ph:trash-fill"></span></button>`;
            } else {
                actionBtnHTML = `<button class="btn-action btn-check" aria-label="Complete Task" title="Complete" onclick="window.toggleTaskStatus('${task.id}', 'Done')"><span class="iconify" data-icon="ph:check-bold"></span> ${btnDoneTxt}</button>
                                 <button class="btn-action" aria-label="Edit Task" title="Edit Task" style="background: var(--primary); color: #fff;" onclick="window.editTask('${task.id}')"><span class="iconify" data-icon="ph:pencil-simple-fill"></span></button>
                                 <button class="btn-action btn-delete" aria-label="Delete Task" title="Delete Task" onclick="window.deleteTask('${task.id}')"><span class="iconify" data-icon="ph:trash-fill"></span></button>`;
            }
    ...
    window.editTask = function(taskId) {
        ...
        const lang = window.app ? window.app.currentLang : 'en';
        document.getElementById('taskModalTitle').innerHTML = `<span class="iconify" data-icon="ph:pencil-simple-fill"></span> ${lang === 'pt' ? 'Editar Tarefa' : 'Edit Task'}`;
    ...
    function bindCrudModal() {
        ...
        btnOpenAddTask.addEventListener('click', () => {
            ...
            const lang = window.app ? window.app.currentLang : 'en';
            document.getElementById('taskModalTitle').innerHTML = `<span class="iconify" data-icon="ph:check-square-fill"></span> ${lang === 'pt' ? 'Criar Nova Tarefa' : 'Create New Task'}`;
        ...
        form.addEventListener('submit', async (e) => {
            ...
            if (!form.checkValidity()) {
                window.showToast("Please fill all required fields correctly.", "warning");
                form.reportValidity();
                return;
            }
